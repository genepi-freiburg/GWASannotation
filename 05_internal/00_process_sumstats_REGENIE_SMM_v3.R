...
#devtools::load_all("/data/programs/pipelines/genepicoloc/source_package/genepicoloc_0.9.1.01")
devtools::load_all("/data/programs/pipelines/genepicoloc/custom_scripts/backup/genepicoloc_dev_smm")
#library(genepicoloc)
library(data.table)
...
cat("number of SNPs original file", nrow(sumstats))
print("head gwas")
head(sumstats)
...
if(genome_build=="hg37"){
 ...
 #keep only chr 1-22
 cat("keeping chr 1-22 and X \n ")
 sumstats_1 <- sumstats_1[sumstats_1$CHR %in% c(1:22, "X"), ]
