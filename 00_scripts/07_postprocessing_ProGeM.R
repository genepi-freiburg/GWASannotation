...
option_list <- list(
  make_option("--ProGeM_dir", action="store", default=NA, type='character', help="ProGeM_dir full path [required]"),
  make_option("--pops.file", action="store", default=NA, type='character', help="pops.file full path [required]"),
  make_option("--sentinel.file", action="store", default=NA, type='character', help="sentinel full path [required]"))

opt <- parse_args(OptionParser(option_list=option_list))

# Reading and processing for bottom_up, top_down, and pops
bottom_up.file <- paste0(opt$ProGeM_dir, "OUTPUT_bottom_up_summary.txt")
top_down.file <- paste0(opt$ProGeM_dir, "OUTPUT_top_down_scores.txt")
pops.file <- opt$pops.file
tophit.file <- opt$sentinel.file
summary <- paste0(opt$ProGeM_dir, "ProGeM_summary.txt")
if (FALSE){ #testing only
    ProGeM_dir <- "/data/programs/pipelines/GWASannotation/02_output/test/ProGeM/"
    pops.file <- "/data/programs/pipelines/GWASannotation/02_output/test/test_PoPS.RData"
    sentinel.file <- "/data/programs/pipelines/GWASannotation/02_output/test/test_sentinel.txt"
    bottom_up.file <- paste0(ProGeM_dir, "OUTPUT_bottom_up_summary.txt")
    top_down.file <- paste0(ProGeM_dir, "OUTPUT_top_down_scores.txt")
    summary <- paste0(ProGeM_dir, "ProGeM_summary.txt")
...
write_xlsx(x=m, path = paste0(opt$ProGeM_dir, "ProGeM_sentinel.xlsx"), format_headers = TRUE)
